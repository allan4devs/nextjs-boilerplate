    ...
    <>
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href="#registro"
                className="inline-flex min-h-14 items-center gap-2 bg-[#f6c400] px-6 font-black uppercase text-black transition hover:bg-white"
              >
      ...
      <section className="border-b border-white/10 bg-[#0e0e0e] px-5 py-12 sm:px-8">
        <div className="mx-auto max-w-7xl">
          <p className="text-xs font-black uppercase tracking-[.2em] text-[#f6c400]">
            Elegí cuándo venir
          </p>
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            {SCHEDULE.map((item) => (
              <div key={item.day} className="border border-white/10 bg-white/[.04] p-5">
                <p className="font-black uppercase">{item.day}</p>
                <p className="mt-2 font-bold text-white/55">{item.hours}</p>
              </div>
            ))}
          </div>
        </div>
      </section>
      <section className="scroll-mt-20 border-b border-white/10 px-5 py-14 sm:px-8">
