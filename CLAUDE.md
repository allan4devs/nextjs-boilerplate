...
- **Recepción:** Valeska y Victoria son las recepcionistas de planta. El mostrador (`/recepcion`) se abre eligiendo operador y digitando un **PIN de mostrador propio**, que se crea en el acto la primera vez (se confía en la presencia física; sin código previo). Ese PIN vive en Mongo (`xtreme_gym_staff_reception_pins`), es **independiente del código de admin** y nunca concede Admin OS. Kengie (entrenador) y Allan (super admin) también pueden atender el mostrador con su PIN de mostrador: entran como `role: "reception"`, no como su rol del directorio. La lista de operadores es `RECEPTION_OPERATOR_IDS` en `lib/xtreme/staff-directory.ts`. Un super admin restablece cualquier PIN de mostrador desde el panel admin (resumen). El código de recepción compartido (`XTREME_RECEPTION_CODE`) queda como respaldo.
