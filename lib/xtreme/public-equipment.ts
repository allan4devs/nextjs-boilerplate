...
export type PublicEquipment = Pick<EquipmentAssetDoc, "id" | "area" | "kind" | "code" | "name" | "location" | "status" | "machineGuideId">;

export async function getPublicEquipment(): Promise<{ inventory: PublicEquipment[]; source: "shared" | "fallback" }> {
  const project = ({ id, area, kind, code, name, location, status, machineGuideId }: PublicEquipment): PublicEquipment => ({ id, area, kind, code, name, location, status, ...(machineGuideId ? { machineGuideId } : {}) });
  try {
    const db = await getDb();
    const saved = await db.collection<EquipmentAssetDoc>(EQUIPMENT_ASSETS_COLLECTION).find({}, {
      projection: { _id: 0, id: 1, area: 1, kind: 1, code: 1, name: 1, location: 1, status: 1, machineGuideId: 1 },
    }).toArray();
