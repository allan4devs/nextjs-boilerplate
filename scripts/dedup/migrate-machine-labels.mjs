...
const kind = process.argv.includes("--benches") ? "bench" : "machine";
...
try {
  ...
  const rows = await assets.find({ kind }, { projection: { id: 1, name: 1 } }).toArray();
  ...
  const planChanges = (await plans.find({}).toArray()).flatMap((saved) => {
    ...
    for (const [id, placement] of Object.entries(nextPlan.placements ?? {})) {
      if (ids.has(id) && typeof placement.label === "string") placement.label = names.migrateMachineName(placement.label);
    }
    return JSON.stringify(nextPlan) === JSON.stringify(saved.plan) ? [] : [{ saved, nextPlan }];
  });
  console.log(JSON.stringify({ mode: apply ? "apply" : "dry-run", kind, assetsReviewed: rows.length, namesToUpdate: changes.length, plansToUpdate: planChanges.length }));
  if (apply && (changes.length || planChanges.length)) {
    const backup = await db.collection("xtreme_gym_machine_label_migrations").insertOne({
      migration: `recognizable-${kind}-names-v1`, createdAt: new Date(), names: changes, plans: planChanges.map(({ saved }) => saved),
    });
